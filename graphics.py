...
class GraphWin(tk.Canvas):
    ...
    def __init__(self, title="Graphics Window",
        ...
        if(sys.platform == 'darwin'): # if using OSX
            ...
            self.bind("<B2-Motion>", self._onRightDrag)
            self.bind("<ButtonRelease-2>", self._onRightRelease)
        else:
            self.bind("<Button-3>", self._onRightClick)
            self.bind("<B3-Motion>", self._onRightDrag)
    ...
    def checkRightClick(self):
        if self.isClosed():
            raise GraphicsError("checkRightClick in closed window")
        self.update()
        if self.rclickX != None and self.rclickY != None:
            x, y = self.toWorld(self.rclickX, self.rclickY)
            self.rclickX = None
            self.rclickY = None
        else:
            return None
    ...
    def checkRightDrag(self):
        if self.isClosed():
            raise GraphicsError("checkRightDrag in closed window")
        self.update()
        if self.rdragX != None and self.rdragY != None:
            x, y = self.toWorld(self.rdragX, self.rdragY)
        else:
            return None
        return Point(x, y)
    ...
    def checkMouseHover(self):
        if self.isClosed():
            raise GraphicsError("checkMouseHover in closed window")
        self.update()
    ...
    def checkMouseDrag(self):
        if self.isClosed():
            raise GraphicsError("checkMouseDrag in closed window")
        self.update()
    ...
    def checkAllKeys(self):
        if self.isClosed():
            raise GraphicsError("checkAllKeys in closed window")
        self.update()
...
class Text(GraphicsObject):
    ...
    def setFace(self, face):
        if face in ['helvetica','arial','caveat','times new roman']:
...
class Entry(GraphicsObject):
    ...
    def setFace(self, face):
        if face in ['helvetica','arial','caveat','times roman']:
